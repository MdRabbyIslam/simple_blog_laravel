...
<nav class="navbar navbar-expand-lg navbar-light fixed-top" id="mainNav">
    <div class="container">
        <a class="navbar-brand" href="index.html">RafuSoft Blog</a>
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            <ul class="navbar-nav ml-auto">
                ...
                <ul class="navbar-nav w-75 d-flex justify-content-between">
                    <?php
                    $navLinks = ["/"=>"Home",'about'=>"About",'write'=>"Write","contact"=>"Contact",];
                    foreach ($navLinks as $route=>$link)
                    {?>
                    <li class="nav-item">
                        <a class=" {{ request()->is("$route")?"my-active":"" }}" href="{{($route=="/")?url('/'):route($route)}}">{{$link}}</a>
                    </li>
                    <?php
                    }?>
                </ul>
